...
- Totalhaveri i LLM-delen är däremot inte tyst: `classify()` fyller i en `ClassifyStats`
  (klassificerade, cache, misslyckade, uppskjutna, ev. auth-fel, senaste feltext) och `main.py`
  kallar körningen degraderad så fort uppdrag behövde LLM men 0 lyckades, oavsett orsak:
  auth-fel, allt uppskjutet, allt failat eller en blandning. Larmmailet visar `auth_error` om
  den finns, annars `last_error`. Då skickas ett varningsmail via `notify.send_alert()` och
  processen avslutar med exitkod 1, efter att digest, state och vanligt mail är klara.
  Commit-steget i `daily-digest.yml` har `if: always()` så filerna ändå committas medan jobbet
  blir rött.
